...
export function initData() {
    ...
    const isCreate = computed(() => !eventModule.selectedEvent?.id);
    ...
    const onSubmit = handleSubmit(async (values) => {
        ...
        let response;
        const eventId = eventModule.selectedEvent?.id;
        ...
        if (response.success) {
            ...
            loading.close();
            await eventModule.setIsShowEventFormPopUp(false);
        } else {
    ...
    const openPopup = async () => {
        if (!isCreate.value) {
            ...
            const eventDetail = (await eventService.getDetail(
                eventModule.selectedEvent?.id || 0,
